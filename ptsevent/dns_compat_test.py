...
#    print ptsevent.dns_resolve_reverse('2001:6b0:1:ea:202:a5ff:fecd:13a6')
...
def FakeDnsResolveIpv4(name):
  ...
  if values is None:
    raise ptsevent.DnsLookupError(-3, 'fake error')  # name does not exist
  return ptsevent.dnsresult(1, 1, values)
...
def Wrap(function, *args):
  try:
    ...
    if e.args[0] == ptsevent.HERROR_HOST_NOT_FOUND:
...
class DnsCompatTest(unittest.TestCase):
  def setUp(self):
    assert callable(getattr(ptsevent, 'dns_resolve_ipv4', None))
    ptsevent.dns_resolve_ipv4 = FakeDnsResolveIpv4
    ptsevent.names_by_ip.clear()
    ptsevent.names_by_nameip.clear()
  ...
  def testEtcHostsDicts(self):
    ...
    self.assertEqual(
        {'1.2.3.4': items1, '127.0.0.1': items2,
         '1.2.3.5': items3}, ptsevent.names_by_ip)
    self.assertEqual(
        {'1.2.3.4': items1, 'bogus1.there': items1, 'bogus2': items1,
         ...
         '127.0.0.1': items2}, ptsevent.names_by_nameip)
